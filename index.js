...
function startSYloveBot(token) {
    try {
        // 🔥 FIX: Webhook delete karo aur polling start karo
        const S7 = new SY(token, { polling: true });
        
        // Webhook force delete
        S7.deleteWebhook().then(() => {
            log('info', 'SYSTEM', '✅ Webhook deleted successfully!');
        }).catch((err) => {
            log('error', 'SYSTEM', '⚠️ Webhook deletion failed: ' + err.message);
        });
...
└──────────────────────┘`;
                ...
                await S7.answerCallbackQuery(query.id, { text: '❌ Error processing request', show_alert: true });
            }
        });
