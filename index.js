...
function startSYloveBot(token) {
    try {
        ...
        // 🔥 Force delete webhook on startup
        S7.deleteWebhook().catch(err => console.log('Webhook delete error:', err.message));
...
└────────────────────────┘`;
        ...
        // ==================== CALLBACK QUERY HANDLER (FINAL FIX) ====================
        S7.on('callback_query', (query) => {
            ...
            // ✅ IMMEDIATE ACKNOWLEDGE (Sab se zaroori step)
            S7.answerCallbackQuery(query.id).catch(err => console.log('Answer error:', err));
...
│➻ /crashfinity [num]
└──────────────────────┘`;
                S7.editMessageText(androidText, {
                    chat_id: chatId,
                    message_id: messageId,
                    parse_mode: 'HTML',
                    reply_markup: {
